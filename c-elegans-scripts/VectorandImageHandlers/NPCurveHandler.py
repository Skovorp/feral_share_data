...
class NPCurveHandler():
    def __init__(self):
        pass
    def get_curve_length(self,ordered_curve_coords):
        return np.sum(np.linalg.norm(np.diff(ordered_curve_coords, axis = 0),axis=1))

    def get_even_points_along_curve(self, ordered_curve_coords, n_pts_to_sample = 12): #get_even_points_along_side
    ...
    def interpolate_data_at_timepoints(self, data, evenly_spaced_times, frame_times):#actual_times_csv):
...
#        frame_times = pd.read_csv(actual_times_csv, sep=',', usecols=['frame_time']).to_numpy().flatten()
    ...
    def get_segment_coords_between_two_indices(self, first_i, second_i, ordered_curve_coords):#get_side_coords_from_contour
        if first_i==second_i:
        ...
        return segment_1, segment_2
    def find_nearest_coordinate_pair_in_list(self, coord_pair, coords_list):
        return np.argmin(np.linalg.norm(coords_list-coord_pair, axis = 1))
    
    def get_evenly_spaced_timepoints_along_array(self, org_frame_times, fps):
    ...
    def dot_product(self, vec1,vec2, decimals_to_round_to = 3 ):
        y1 = vec1[:,1]
        x1 = vec1[:,0]
    ...
    def get_signed_angle_between_norm_vectors(self,vec1, vec2):
    ...
    def get_signed_supplementary_angles_between_coords1(self,coords_list):
        ...
        angle = self.get_signed_angle_between_norm_vectors(vec2, vec1)
    ...
    def convert_radians_to_degrees(self,radians_vec):
        return np.degrees(radians_vec)

    def get_two_furthest_pts_index(self, np_pts_list):
        #def option2(r):
        dists = scipy.spatial.distance.pdist(np_pts_list, 'cityblock')
        dist_i = np.argmax(dists)
        n_extrema = np_pts_list.shape[0]
        pt_i1, pt_i2 = self.recover_indices_from_scipy_condensed_matrix_index(dist_i, n_extrema)
        return pt_i1, pt_i2

    def recover_indices_from_scipy_condensed_matrix_index(self, i, n_nodes):
        pairs = list(itertools.combinations(range(n_nodes),2))
        return pairs[i]
    
    def get_start_end_is_of_ones_in_binary_array(self, binary_array):
        '''
    ...
    def get_durations_of_ones_in_binary_array(self, chunk_start_is, chunk_end_is):
        return chunk_end_is-chunk_start_is
    

    def get_curve_length(self, array):
    ...
    def get_midline_length_from_ordered_coords(self, ordered_midline_coords):
